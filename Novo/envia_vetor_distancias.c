 ...
 void* envia_vetor_distancias(void *args) {
   ...
   while (1) {
     ...
     for (i = 0; i < quantidade_vizinhos; i++) {
       ...
       //printf("Enviando para %d: ", argumentos->vizinhos[i].id);
       ...
       for (j = 0; j < TAMANHO_MENSAGEM_PACOTE / 4; j++) {
         ...
         //if (j < 7) printf(" %d", argumentos->tabela_roteamento[nodo_atual][j] == INFINITO ? -1 : argumentos->tabela_roteamento[nodo_atual][j]); //DEBUG
       }
       //printf("\n"); //DEBUG
    ...
    //imprime_tabela_roteamento(argumentos->tabela_roteamento, argumentos->vetor_saltos); // DEBUG
